...
**continued pretraining** (계속 사전학습 / continual learning). 같은 CausalLM task,
...
continued pretraining) 비교가 *데이터·trainer·collator·loss 통제하고 본체·토크나이저·
lr 만 변하는* 격리 실험.
"""
import json
...
# ----- 1. 제목 -----
md(r"""# Chapter 25. `gpt2` Continued Pretraining — 대규모 사전학습 모델을 TinyStories 로 계속 학습

**목표**: Phase 4 의 두 번째 챕터. Ch 24 에서 *random init 작은 GPT (약 3M params) 를 TinyStories 로 from scratch 사전학습* 했다면, 이번엔 **OpenAI `gpt2` (124M params, WebText 약 40GB 사전학습된 본체)** 를 *같은 TinyStories 데이터* 로 **continued pretraining** (계속 사전학습 / continual learning) 합니다. **같은 CausalLM task, 같은 LM head, 같은 collator, 같은 loss** — 변하는 건 *모델 로드 한 줄 + 학습률* 뿐. 그게 GPT 시대 *학습 단계 2 (continued pretraining)* 의 본질입니다.

**환경**: Google Colab **T4 GPU 필수**.

**예상 소요 시간**: 약 15-20분 (데이터 로드 약 2분 + gpt2 로드·토큰화 약 2분 + 학습 전 baseline generation 약 1분 + continued pretraining 약 8-10분 + 학습 후 generation + 3-way 비교 약 2분)
...
6. 🚀 **실습**: TinyStories 30K → `gpt2` 로드 → 학습 전 generation → continued pretraining → 학습 후 generation
7. 🆚 **3-way generation 비교** — Ch 24 (scratch) vs Ch 25 BEFORE (gpt2 그대로) vs Ch 25 AFTER (continued pretraining)
8. 📦 **등장 라이브러리** / 🎯 **체크포인트** / ❓ **FAQ** (답변 포함)

---

> 📒 **사전 학습 자료**: Ch 24 (영어 GPT scratch + TinyStories). 본 챕터는 Ch 24 와 *데이터·trainer·collator·loss 모두 같고 본체 출발점·토크나이저·lr 만 다른* 격리 실험. *trainer 코드 차이가 극단적으로 적음* — 그게 *학습 단계 2 (continued pretraining)* 의 본질입니다.""")
...
| **25 ← 여기** | **`gpt2` (124M, OpenAI WebText 사전학습)** | **BPE (gpt2 그대로, vocab 50,257)** | **TinyStories 30K (Ch 24 와 동일)** | **`Linear(H, V)` (LM head 그대로)** | **`CrossEntropyLoss` (next-token) — *continued pretraining***  |
...
| 학습률 | 3e-4 (scratch 표준) | **2e-5** (continued pretraining 표준) | **다름** |
| 학습 step | 약 1,500 | **약 500-800** | **다름** (본체 이미 학습됨) |

> **세 줄 차이가 곧 *학습 단계 2 (continued pretraining)* 의 정의** — 같은 task, 같은 collator, 같은 loss, 같은 trainer. *모델 로드 한 줄 + 토크나이저 한 줄 + lr 한 숫자* 만 바꾸면 됩니다.""")

# ----- 3. 변경점 (Diff from Ch 24) -----
md(r"""## 🔄 변경점 (Diff from Ch 24)

| 축 | Ch 24 (영어 GPT scratch) | Ch 25 (본 챕터, gpt2 continued pretraining) |
|---|---|---|
...
> **핵심**: *Ch 24 ↔ Ch 25 는 데이터·trainer·collator·loss 모두 같고 본체 출발점·토크나이저·lr 만 다름*. **trainer 코드 차이가 극단적으로 적음** — 그게 GPT 시대 학습 단계 2 (continued pretraining) 의 본질. *task adaptation 의미의 fine-tune 이 아닙니다* — head 바뀌지 않고, task (next-token 예측) 바뀌지 않습니다. 데이터만 바뀝니다.""")

# ----- 4. GPT 시대 학습 4단계 표 -----
md(r"""## 🎯 GPT 시대 학습 4단계 — 본 챕터의 위치 (단계 2)

Ch 24 에서 도입한 GPT 시대 학습 4단계 표의 *단계 2 (continued pretraining)* 가 본 챕터입니다.

| 단계 | 정확 용어 | 의미 | `labels = -100` 자리 | 본 커리큘럼 | 본 챕터? |
|---|---|---|---|---|---|
| 1 | **Pretraining** (사전학습) | 일반 코퍼스 위에 random init 본체부터 학습 | pad 만 | Ch 24, Ch 26 | |
| 2 | **Continued pretraining** (계속 사전학습 / continual learning) | *사전학습된 본체* 를 *새 데이터* 로 *같은 CausalLM task* 더 학습. **head 그대로, task 그대로, 데이터만 새로** | pad 만 (단계 1 과 동일) | **Ch 25 ← 여기** | ✅ |
...
- **`continued pretraining` 그 자체** — *사전학습된 본체 + 새 도메인 데이터 + 같은 CausalLM task* 의 조합. *데이터만 바뀐 단계 1 의 연장*
...
| Continued pretraining 후 (수백 step) | $0.10$ - $0.20$ 범위 | **1.6 - 2.3** ← *우리 도달점* |
...
# Ch 25 (continued pretraining) - 단 두 줄로
tokenizer = AutoTokenizer.from_pretrained("gpt2")
tokenizer.pad_token = tokenizer.eos_token
model = AutoModelForCausalLM.from_pretrained("gpt2")
```

> *trainer·collator·loss 는 같음* — *모델 로드 한 줄 + 토크나이저 한 줄* 로 학습 단계 2 (continued pretraining) 에 진입합니다. 그게 본 챕터의 메시지.""")
...
같은 prompt 3개로 *gpt2 학습 직전 (BEFORE)* generation 을 기록 — 학습 후 (§6) 와 나란히 비교해 *continued pretraining 이 본체에 어떤 변화를 주는가* 를 직접 봅니다.""")
...
before_outputs = []
print("=" * 70)
print("BEFORE continued pretraining - gpt2 pretrained on WebText, as-is")
print("=" * 70)
for p in PROMPTS:
...
> 이 차이가 *학습 시작점의 차이*. Ch 25 는 *random 에서 시작하지 않습니다* — *이미 잘 만들어진 본체* 에서 시작해 *TinyStories 풍 적응* 만 더하는 게 학습 단계 2 (continued pretraining) 의 본질.""")

# ----- 12. Continued Pretraining -----
md(r"""## 5. Continued Pretraining — *trainer 코드는 Ch 24 와 거의 동일*

Ch 24 와 *완전히 같은 구조* 의 `Trainer` 코드. 변하는 곳은 **lr (`3e-4 → 2e-5`)** 와 **step 수 (`1500 → 약 500-800`)** 두 곳.

### 왜 lr 가 작아지는가 — `2e-5` 의 정확한 의미

Ch 24 (scratch) 의 lr `3e-4` 는 *random init 본체* 가 *빠르게 의미 있는 표상* 을 학습하기 위한 표준 값. Ch 25 (continued pretraining) 는 *이미 학습된 본체* 라 *큰 lr 면 사전학습된 표상이 망가질 위험* — **catastrophic forgetting**. `2e-5` 는 HF 의 continued pretraining / fine-tuning 표준 lr 중 가장 작은 쪽으로, *사전학습 표상 보존* 을 우선.
...
ax1.set_title("gpt2 continued pretraining on TinyStories - loss")
...
# ----- 13. 학습 후 generation -----
md(r"""## 6. 학습 *후* generation — *continued pretraining 의 효과*

같은 `PROMPTS / GEN_KWARGS` 로 학습 후 모델에서 다시 생성. *BEFORE (gpt2 그대로) → AFTER (continued pretrained on TinyStories)* 비교가 *학습 단계 2 가 본체에 새긴 도메인 적응* 을 직접 드러냅니다.""")

code(r"""torch.manual_seed(SEED)
model.eval()
after_outputs = []
print("=" * 70)
print("AFTER continued pretraining - gpt2 + TinyStories 30K")
print("=" * 70)
for p in PROMPTS:
    text = generate_text(model, p, **GEN_KWARGS)
...
print("=" * 78)
print("Ch 25 BEFORE (gpt2 as-is) vs AFTER (gpt2 + TinyStories continued pretrain)")
print("=" * 78)
...
md(r"""**해석 가이드 — continued pretraining 의 도메인 적응 효과**
...
> 본체는 *같은 124M params 모델* 이고, *한 줄 코드 차이 (lr) + 한 epoch 의 데이터* 만으로 *generation 톤 자체가 도메인 적응*. 그게 *continued pretraining 의 정량적 가치* — *task adaptation 의미의 fine-tune (head 교체 / 새 loss) 이 아닙니다*, *같은 task 의 데이터만 바뀐 단계 1 의 연장*.""")
...
| **Ch 25 AFTER** | 124M params (gpt2) | **WebText 약 40GB** | **1 epoch continued pretraining** |""")
...
- **Ch 25 AFTER (gpt2 + TinyStories continued pretrain)**: *동화 풍 + 자연스러움 + 일반 도메인 어휘력* 결합. *작은 from-scratch 의 도메인 특화 + 큰 사전학습 모델의 어휘 폭* 이 모두
...
| 항목 | Ch 24 (3M scratch) | **Ch 25 (124M continued pretrain)** |
|---|---|---|
...
> Ch 25 의 결과만 보면 *대규모 사전학습 + continued pretraining* 이 압도적으로 보이지만, *3M params + WebText 사전학습* (가상의 비교군) 이라면 어떻게 될까요 — *모델 크기와 사전학습 데이터를 분리하는 비교* 는 본 챕터의 셋업으로는 어렵습니다. 그게 *실험 설계의 한계* 이자 *학습 단계 2 의 실용성* — 실무는 보통 *큰 사전학습 모델을 그대로 가져와 continued pretraining* 하는 게 비용 대비 최선이라.""")
...
### 변형 3. 다른 도메인 데이터 — *continued pretraining 의 일반성*
...
| `warmup_ratio` (vs `warmup_steps`) | epoch 비율 기반 warmup (continued pretraining 표준) | **약간 다름** (Ch 24 는 `warmup_steps=100`) |
| `num_train_epochs` (vs `max_steps`) | epoch 수 기반 학습 (continued pretraining 1 epoch 충분) | **약간 다름** (Ch 24 는 `max_steps=1500`) |
...
3. *Continued pretraining* (단계 2, 본 챕터) 과 *SFT* (단계 3, Ch 27) 의 가장 큰 차이는 *`labels = -100` 자리* 입니다. 본 챕터의 collator 가 만드는 `labels` 패턴과, Ch 27 에서 등장할 `labels[:prompt_len] = -100` 한 줄의 차이를 직접 비교해 설명해 보세요.
...
### Q1. (이론) *Continued pretraining* 이 정확히 무엇인가요? *fine-tune* 과 어떻게 다른가요?

**Continued pretraining** (계속 사전학습 / continual learning) 은 GPT 시대 학습 4단계 중 *단계 2*:

| 단계 | 정확 용어 | 의미 |
|---|---|---|
| 1 | **Pretraining** | random init 본체 + 일반 코퍼스 |
| 2 | **Continued pretraining** ← *본 챕터* | *사전학습된 본체* + *새 도메인 데이터*. **head 그대로, task 그대로, loss 그대로** |
...
- **GPT 시대 continued pretraining**: *데이터 적응* — 본체 + 같은 head + 같은 loss + 새 데이터. **본 챕터.**
- **GPT 시대 SFT**: *행동 정렬* — 본체 + 같은 head + 같은 loss + instruction-response 데이터 + `-100` 마스킹. Ch 27.

정확히 부르자면 본 챕터는 *fine-tune 이 아니라 continued pretraining*. *task 가 안 바뀌고 데이터만 바뀐* 게 핵심.
...
# Ch 25 (continued pretraining) - 작은 lr 로 표상 보존
TrainingArguments(learning_rate=2e-5, ...)
```

HF 의 continued pretraining / fine-tuning 표준 lr 범위: `1e-5` - `5e-5`. SFT (Ch 27) 도 비슷한 범위.
...
**실용적 결론**: 실무에서는 (b)(c) 가 *비용 대비 비효율* 이라 (d) 패턴이 표준. *대규모 사전학습 모델을 가져와 작은 도메인 데이터로 continued pretraining* — 본 챕터의 패턴이 그 자체로 *실무 표준 레시피*.
...
Ch 25 에서는 *짧은 (1 epoch) continued pretraining + 작은 lr (`2e-5`)* 라 *catastrophic forgetting 이 강하지 않음*. 다만 *변형 1 (epoch 늘리기)* 또는 *큰 lr (`1e-3` 등)* 을 시도하면 다음 신호가 보입니다:
...
> 그게 *학습 단계 2 (continued pretraining)* 의 미적 본질 — *trainer / collator / loss 코드 재사용*. 단계 3 (SFT, Ch 27) 에서도 *대부분 같습니다*, 다만 *collator 가 `labels[:prompt_len] = -100` 마스킹* 한다는 점이 추가될 뿐.

### Q7. (이론) 다음 챕터 (Ch 26 한국어 GPT scratch) 와의 관계는?

Ch 26 는 *Ch 24 의 한국어판* — *작은 GPT + 한국어 TinyStories-Korean + BPE 직접 학습* 패턴. *Ch 25 의 한국어판 (한국어 사전학습 GPT + continued pretraining)* 이 *아닌* 이유:
...
| **Ch 25 ← 여기** | **2** | **`gpt2` 124M** | **TinyStories (동일)** | **단계 2: continued pretraining** |
...
> **변하는 축** (Ch 25 → Ch 26): *언어 + 학습 단계* 두 축. *직접 짝* 은 Ch 24 ↔ Ch 26 (같은 단계 1, 언어만 다름) / Ch 25 ↔ (가상의 한국어 continued pretraining, KoGPT2 + 한국어 도메인 데이터) 가 됩니다.""")
...
# ----- README.md 작성 -----
README = """# 25_gpt2_continual_pretrain — gpt2 (124M) Continued Pretraining (Phase 4 단계 2)
...
| **2** | **Continued pretraining** (계속 사전학습 / continual learning) | **✅ ← 여기** | **Ch 25** |
...
- **`tokenizer.pad_token = tokenizer.eos_token`** — gpt2 의 pad 컨벤션
- **lr `2e-5`** — continued pretraining 표준 (Ch 24 의 `3e-4` 보다 약 15배 작음). *catastrophic forgetting 방지*
...
- **3-way generation 비교** — Ch 24 (3M scratch) vs Ch 25 BEFORE (gpt2 그대로) vs Ch 25 AFTER (continued pretrain). *모델 크기와 사전학습 효과는 분리 불가능* 의 정량 표시
- **Catastrophic forgetting** — 긴 학습 / 큰 lr 일 때 사전학습된 일반 도메인 능력이 손실되는 현상. 짧은 학습 + 작은 lr 로 완화
- **Continued pretraining ↔ SFT (Ch 27) 의 정확한 경계** — `labels = -100` 자리가 *pad 만 (단계 2)* vs *prompt 부분 (단계 3)*
...
## 모델
**`AutoModelForCausalLM.from_pretrained("gpt2")`** — `n_layer=12, n_head=12, n_embd=768, n_positions=1024`. 약 **124M params** (Ch 24 의 약 40배). WebText 약 40GB 로 사전학습된 본체 그대로 로드 → continued pretraining.
...
## 환경
Google Colab **T4 GPU 필수**. 약 15-20분 (데이터 로드 약 2분 + gpt2 로드·토큰화 약 2분 + 학습 전 generation 약 1분 + continued pretraining 약 8-10분 + 학습 후 generation + 3-way 비교 약 2분).
...
| **25** | **`gpt2` (124M, WebText 사전학습)** | **BPE (gpt2 그대로, vocab 50,257)** | **TinyStories 30K (Ch 24 와 동일)** | **Linear(H, V) (LM head 그대로)** | **CE (next-token) — *continued pretraining*** |
