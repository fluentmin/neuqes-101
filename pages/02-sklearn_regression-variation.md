...
정규화 공간의 MSE(0.0973)는 라벨을 4로 나눠 압축한 만큼 작아 보일 뿐이고, 원래 별점 공간으로 되돌리면 정규화 없이 학습한 값(1.5565)과 정확히 같습니다. `LinearRegression`은 정규방정식으로 풀어 라벨 스케일링이 결과를 바꾸지 않음을 보여줍니다.
...
정답을 [0, 1]로 압축해 학습했는데도 예측은 -0.638~1.538로 여전히 범위를 벗어나고, 약 17.7%(85+92개)가 [0, 1] 밖에 놓입니다. 라벨을 아무리 스케일링해도 가중합만으로는 출력을 가둘 수 없다는 것 — 즉 활성화 함수가 필요하다는 다음 챕터의 동기를 확인합니다.
