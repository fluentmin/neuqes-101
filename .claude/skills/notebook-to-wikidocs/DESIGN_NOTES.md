...
### 출력을 코드와 시각적으로 구분 (2026-06-09, PR #1 머지 후)
WikiDocs 렌더에서 코드 펜스와 출력 펜스가 같은 회색 박스로 보여 헷갈린다는 피드백.
**결정: 출력 전체를 블록인용(`> `)으로 감싼 카드**로 렌더(왼쪽 세로바로 코드와 구분, 항상 보임).
- 라벨은 `> **▶ 실행 결과**`.
- 블록인용 안에 **마크다운 표를 넣으면 깨질 수 있어**, 표 출력도 `text/plain`(콘솔 표현)을
  코드펜스로 통일(중첩 안전). HTML 표는 text/plain 없을 때만 정렬 텍스트로 폴백.
- 이미지는 `> ![output](../assets/..)`.
- ⚠️ 펜스-인-블록인용은 CommonMark 표준이나 WikiDocs 실제 렌더 확인 필요(사용자 검증 대기).
