...
| 28 | — | [![Open](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/28_ppo/28_ppo.ipynb) | SFT base + RM + value head + ref | BBPE | prompts + reward model | actor logits + critic value | softmax (정책) | `PPO clip objective` (`PPOTrainer`) | rollout + reward |
...
- **Phase 4 (Ch 24-30) — GPT/LLM (autoregressive)**: encoder(BERT)에서 decoder(GPT)로. 사전학습 & 활용(24 TinyStories scratch → 25 gpt2 fine-tune → **26 한국어 tiny GPT scratch — Ch 24의 한국어판, `g0ster/TinyStories-Korean`**) → instruction tuning & alignment(27 SFT → 28 PPO → 29 DPO → 30 GRPO). 부록 A1 (`27_sft/appendix_agentic.ipynb`) — agentic / function-calling SFT.
