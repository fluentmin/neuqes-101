...
| 25 | — | [![Open](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yoon-gu/neuqes-101/blob/master/25_gpt2_continual_pretrain/25_gpt2_continual_pretrain.ipynb) | gpt2 (사전학습) — **continued pretraining** (계속 사전학습 / continual learning, *같은 CausalLM task 를 새 데이터로 더 학습*. SFT 아님) | BPE (GPT2) | TinyStories (Ch 24와 동일) | `Linear(H,V)` (LM head 그대로) | softmax | `CrossEntropyLoss` (next-token) | shifted token ids |
...
- **Phase 4 (Ch 24-30) — GPT/LLM (autoregressive)**: encoder(BERT)에서 decoder(GPT)로. *GPT 시대의 학습 4단계* — **사전학습 (Ch 24 scratch, Ch 26 한국어 scratch) → continued pretraining (Ch 25 gpt2 + TinyStories, 계속 사전학습) → SFT (Ch 27 KoGPT2 + KoAlpaca, instruction tuning) → alignment (Ch 29 DPO → Ch 30 GRPO)**. 사이에 **분야별 벤치마크 평가 (Ch 28: MMLU/HellaSwag/GSM8K/HumanEval + KMMLU/HAERAE/LogicKor, `lm-evaluation-harness`)**. 부록 A1 (`27_sft/appendix_agentic.ipynb`) — agentic / function-calling SFT. *PPO 는 T4 메모리 한계(actor+critic+RM+ref 4 모델 동시 로드)로 제외 — DPO 가 그 부담 없는 대체.*
